...
// Indexes
PostSchema.index({ tweetIdStr: -1, name: -1, category: -1, correspondDate: -1, userIdStr: -1 });
UserSchema.index({ twitterIdStr: -1 });
TagSchema.index({ twitterIdStr: -1 });
...
// 特定のカテゴリの、該当日だけのデータの件数を取得
PostProvider.prototype.countOnlyCorrespondDate = function(params, callback) {
  Post.count({category: params['name'], correspondDate: params['correspondDate']}, function(err, count) {
    callback(null, count)
  });
};
